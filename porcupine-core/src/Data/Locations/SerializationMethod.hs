...
-- | Data that can be converted to CSV.
...
data CSVSerial a = CSVSerial
  ...
  , csvSerialHasHeader :: Bool
    -- ^ The csv file contains a header (to skip or to read/write)
  , csvSerialDelimiter :: Char
    -- ^ The character (@,@, @\t@, etc.) to use as a field delimiter.
  , csvSerialColumns   :: CSVColumns a
    -- ^ Gives information about how columns should be handled
  }

-- | Embeds knowledge about how we can interpret the columns generated by some
-- datatype
data CSVColumns a where
  ColPositional :: (Csv.ToRecord a, Csv.FromRecord a) => CSVColumns a
  ColNamed :: (Csv.ToNamedRecord a, Csv.FromNamedRecord a, Csv.DefaultOrdered a) => CSVColumns a

instance SerializationMethod (CSVSerial a) where
  getSerialDefaultExt = Just . csvSerialExt

instance (Foldable f) => ToBinaryBuilder (CSVSerial a) (Tabular (f a)) where
  toBinaryBuilder (CSVSerial _ hasHeader delim cols) (Tabular mbHeader dat) =
    ...
    where
      ...
      encField = case cols of
        ColPositional -> CsvBuilder.encodeRecordWith encodeOpts
        ColNamed -> CsvBuilder.encodeDefaultOrderedNamedRecordWith encodeOpts
        
instance FromByteStream (CSVSerial a) (Tabular (V.Vector a)) where
  fromLazyByteString (CSVSerial _ hasHeader delim cols) bs =
    case cols of
      ColPositional -> Tabular Nothing <$>  -- TODO: parse header
                       Csv.decodeWith decOpts (if hasHeader then Csv.HasHeader else Csv.NoHeader) bs
      ColNamed | not hasHeader -> error "CANNOT USE ColNamed on CSV files without headers"
               | otherwise -> do
                   (h, v) <- Csv.decodeByNameWith decOpts bs
                   return $ Tabular (Just $ map TE.decodeUtf8 $ V.toList h) v
    where
      decOpts = Csv.defaultDecodeOptions {Csv.decDelimiter=fromIntegral $ ord delim}

instance FromByteStream (CSVSerial a) (V.Vector a) where
  fromLazyByteString s = fmap tabularData . fromLazyByteString s

instance (Foldable f) => SerializesWith (CSVSerial a) (Tabular (f a)) where
  getSerialWriters = getSerialWriters_ToBinaryBuilder

instance DeserializesWith (CSVSerial a) (Tabular (V.Vector a)) where
  getSerialReaders = getSerialReaders_FromByteStream

-- DEPRECATED
instance DeserializesWith (CSVSerial a) (V.Vector a) where
  getSerialReaders = getSerialReaders_FromByteStream
